...
void BuildWorld(game::World& world, gl::Program* program) {
    ...
    auto octo = new gl::Mesh("octo.obj");
    ...
    shape->Translate(vec3(-1.0f, 2.0f, -4.65f));
    shape->surface = moss;
    shape->color = vec4(0.0f, 1.0f, 0.0f, 1.0f);
    *shape += new physics::Velocity(vec3(0.0f, 0.0f, 0.0f));
    *shape += new physics::Gravity();
    *shape += new geometry::SphereCollider(1.0f);
    world += shape;
    shape = new game::Object(block, program);
    shape->Translate(vec3(0.0f, -2.0f, 0.0f));
    shape->surface = sand;
	*shape += new utility::Motor(vec3(1.0f, 0.0f, 0.0f));
    *shape += new geometry::BoxCollider(6.0f, 0.6f, 3.0f);

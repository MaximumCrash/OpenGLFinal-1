...
"void main() {" "\n"
    ...
    "vec3 screen = vec3(1.0) - color.a * (vec3(1.0) - color.rgb);" "\n"
    "vec4 tint = texture(tx, frag_uv);" "\n"
    "float shade = 0.5 * (-dot(normal, normalize(light_direction)) + 1.0);" "\n"
    "fragColor = vec4(tint.rgb * screen * shade, tint.a);" "\n"
"}";

std::string shader::vPhong =
"";

std::string shader::fPhong =
"";
